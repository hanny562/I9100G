...
static struct pm_qos_request_list *pm_qos_audio_handle;
#endif

//#define SEC_AUDIO_DEBUG
#define HIGH_FREQ_NOISE_PATCH
#define MIN_FREQ_FORCE_SETTING
...
#ifdef MIN_FREQ_FORCE_SETTING
	static int previous_min_freq;
#endif

struct wake_lock snd_soc_wakelock;
		...
		if (card->rtd[i].dai_link->no_pcm) {
			...
			if (num > 0) {
				if (rtd->num_be[substream->stream] == SND_SOC_MAX_BE)
					...
					printk(KERN_ERR "** active path for %s to %s\n", fe_aif, be_aif);
...
int snd_soc_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if(codec_dai->driver->playback.stream_name 
		...
		printk("codec pcm_open %s, %s, %s\n",codec_dai->driver->name, 
			codec_dai->driver->playback.stream_name, codec_dai->driver->capture.stream_name);
	if(cpu_dai->driver->playback.stream_name 
		|| cpu_dai->driver->capture.stream_name)
		printk("cpu pcm_open %s, %s, %s\n", cpu_dai->driver->name, 
			cpu_dai->driver->playback.stream_name, cpu_dai->driver->capture.stream_name);

    if(substream->stream == SNDRV_PCM_STREAM_CAPTURE)
        if(cpu_dai->driver->name && cpu_dai->driver->capture.stream_name)
            if(strcmp(cpu_dai->driver->name,"MODEM") && strcmp(cpu_dai->driver->name,"Voice Capture")){
                #ifdef CONFIG_OMAP_PM
                ret = omap_pm_set_max_mpu_wakeup_lat(&pm_qos_audio_handle, 400);
                #endif
                printk("Capture device open constrain on!! %d\n", ret);
            }
	/* 
	...
	if(rtd->codec && rtd->codec->dapm->bias_level == SND_SOC_BIAS_OFF){
		printk("snd_soc_pcm_open %d\n", rtd->codec->dapm->bias_level);
		if(rtd->codec->driver->set_bias_level)
			rtd->codec->driver->set_bias_level(rtd->codec, SND_SOC_BIAS_STANDBY);
	}
#endif
...
#ifdef MIN_FREQ_FORCE_SETTING			
		previous_min_freq = policy->min;
		policy->min = 600000;
		policy->user_policy.min = 600000;
...
static void close_delayed_work(struct work_struct *work)
{
	...
	wake_unlock( &snd_soc_wakelock);	
	/* 
	 * REVIST: After twl6040 power-on/off sequence finalization. 
	 */
#if 0	
	/* codec power off sequence for power saving*/
	if(rtd->codec && rtd->codec->dapm->bias_level == SND_SOC_BIAS_STANDBY){
		printk("close_delayed_work %s, %s %d\n", 
			codec_dai->driver->name, cpu_dai->driver->name, rtd->codec->dapm->bias_level);
		if(rtd->codec->driver->set_bias_level)
			rtd->codec->driver->set_bias_level(rtd->codec, SND_SOC_BIAS_OFF);
	}
#endif
...
int snd_soc_pcm_close(struct snd_pcm_substream *substream)
{
	 ...
	 */
//	printk("====codec->active =%d==\n",codec->active); // temp patch 
	if ((substream->stream == SNDRV_PCM_STREAM_PLAYBACK) && (codec->active == 0))
//	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
...
#ifdef MIN_FREQ_FORCE_SETTING			
		policy->min = previous_min_freq ;
		policy->user_policy.min = previous_min_freq ;
		cpufreq_update_policy(policy->cpu);
#endif
	}
	...
	if(codec_dai->driver->playback.stream_name 
		|| codec_dai->driver->capture.stream_name)
	printk("codec pcm_close %s, %s, %s\n",codec_dai->driver->name, 
		codec_dai->driver->playback.stream_name, codec_dai->driver->capture.stream_name);

	if(cpu_dai->driver->playback.stream_name 
		|| cpu_dai->driver->capture.stream_name)
	printk("cpu pcm_close %s, %s, %s\n", cpu_dai->driver->name, 
		cpu_dai->driver->playback.stream_name, cpu_dai->driver->capture.stream_name);
    ...
    if(substream->stream == SNDRV_PCM_STREAM_CAPTURE)
        if(cpu_dai->driver->name && cpu_dai->driver->capture.stream_name)
            if(strcmp(cpu_dai->driver->name,"MODEM") && strcmp(cpu_dai->driver->name,"Voice Capture")){
                #ifdef CONFIG_OMAP_PM
                ret = omap_pm_set_max_mpu_wakeup_lat(&pm_qos_audio_handle, -1);
                #endif
                printk("Capture device off constrain off!! %d\n", ret);
            }
no_pcm:
	mutex_unlock(&rtd->pcm_mutex);
	return 0;
...
static int soc_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&snd_soc_wakelock, WAKE_LOCK_SUSPEND, "snd-soc");
	return 0;
}
