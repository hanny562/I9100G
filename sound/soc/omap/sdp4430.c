...
#ifndef CONFIG_MACH_OMAP4_SAMSUNG
...
static int sdp4430_set_power_mode(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	printk("twl6040 set power mode %d\n", ucontrol->value.integer.value[0]);
...
static int main_mic_bias(struct snd_soc_dapm_widget *w,
				struct snd_kcontrol *k, int event)
{
	struct snd_soc_codec *codec = w->codec;
	struct twl4030_codec_audio_data *pdata = dev_get_platdata(codec->dev);

	if(!pdata->mainmic_gpio_cb){
		printk(KERN_ERR "mainmic_gpio_cb is null event %d\n", SND_SOC_DAPM_EVENT_ON(event));			
		return 0;
	}
		
	printk(KERN_WARNING "main_mic_bais event %d\n", SND_SOC_DAPM_EVENT_ON(event));	
	
	if(SND_SOC_DAPM_EVENT_ON(event)){
		pdata->mainmic_gpio_cb(1);
	}else{
		pdata->mainmic_gpio_cb(0);
	}
	return 0;
}
static int sub_mic_bias(struct snd_soc_dapm_widget *w,
				struct snd_kcontrol *k, int event)
{
	struct snd_soc_codec *codec = w->codec;
	struct twl4030_codec_audio_data *pdata = dev_get_platdata(codec->dev);

	if(!pdata->submic_gpio_cb){
		printk(KERN_ERR "sub_mic_bais_cb is null event %d\n", SND_SOC_DAPM_EVENT_ON(event));			
		return 0;
	}
	
	printk(KERN_WARNING "sub_mic_bais event %d\n", SND_SOC_DAPM_EVENT_ON(event));	
	
	if(SND_SOC_DAPM_EVENT_ON(event)){
		pdata->submic_gpio_cb(1);
	}else{
		pdata->submic_gpio_cb(0);
	}
	return 0;
}
...
static const struct snd_soc_dapm_widget sdp4430_twl6040_dapm_widgets[] = {
	SND_SOC_DAPM_MIC("Ext Mic", main_mic_bias),
	SND_SOC_DAPM_MIC("Sub Mic", sub_mic_bias),
...
static int sdp4430_twl6040_init_hs(struct snd_soc_pcm_runtime *rtd)
{
	...
	struct twl4030_codec_audio_data *pdata = dev_get_platdata(codec->dev);
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;
	int hsotrim, dl1_offset, dl2_offset, ret;
	...
	snd_soc_dapm_enable_pin(codec->dapm, "Sub Mic");
	snd_soc_dapm_enable_pin(codec->dapm, "Ext Spk");
	snd_soc_dapm_enable_pin(codec->dapm, "AFML");
	snd_soc_dapm_enable_pin(codec->dapm, "AFMR");
	snd_soc_dapm_enable_pin(codec->dapm, "Aux/FM Stereo Out");
	...
	snd_soc_dapm_ignore_suspend(codec, "Sub Mic");
	...
	if (machine_is_omap_4430sdp() || machine_is_omap4_samsung())
	...
	dl1_offset = (hsotrim & TWL6040_HSRO_MSK) >> 4;
	dl2_offset = hsotrim & TWL6040_HSLO_MSK;
	/* sign extension because McPDM DN_OFST_RX1/RX2 is 5-bits */
	if (dl1_offset & 0x08)
		dl1_offset |= 0x10;
	if (dl1_offset & 0x08)
		dl1_offset |= 0x10;
	omap_mcpdm_update_dc_offset(cpu_dai, dl1_offset, dl2_offset);
...
static int __init sdp4430_soc_init(void)
{
	...
	if (!machine_is_omap_4430sdp() && !machine_is_omap4_panda() && !machine_is_omap4_samsung()) {
