...
static struct omap_volt_data omap44xx_vdd_mpu_volt_data[] = {
	{.volt_nominal = 875000, .sr_errminlimit = 0xF4, .vp_errgain = 0x0C, .abb_type = NOMINAL_OPP},
	{.volt_nominal = 900000, .sr_errminlimit = 0xF4, .vp_errgain = 0x0C, .abb_type = NOMINAL_OPP},
	{.volt_nominal = 975000, .sr_errminlimit = 0xF4, .vp_errgain = 0x0C, .abb_type = NOMINAL_OPP},
	{.volt_nominal = 1150000, .sr_errminlimit = 0xF9, .vp_errgain = 0x16, .abb_type = NOMINAL_OPP},
	{.volt_nominal = 1275000, .sr_errminlimit = 0xFA, .vp_errgain = 0x23, .abb_type = NOMINAL_OPP},
	{.volt_nominal = 1359000, .sr_errminlimit = 0xFA, .vp_errgain = 0x27, .abb_type = FAST_OPP},
	{.volt_nominal = 1360000, .sr_errminlimit = 0xFA, .vp_errgain = 0x27, .abb_type = FAST_OPP},
};
...
static struct omap_vdd_dep_volt omap44xx_vddmpu_vddcore_data[] = {
	{.main_vdd_volt = 875000, .dep_vdd_volt = 942000},
	{.main_vdd_volt = 900000, .dep_vdd_volt = 962000},
	{.main_vdd_volt = 975000, .dep_vdd_volt = 962000},
	{.main_vdd_volt = 1150000, .dep_vdd_volt = 1127000},
	{.main_vdd_volt = 1275000, .dep_vdd_volt = 1127000},
	{.main_vdd_volt = 1359000, .dep_vdd_volt = 1127000},
	{.main_vdd_volt = 1360000, .dep_vdd_volt = 1127000},
	{.main_vdd_volt = 0, .dep_vdd_volt = 0},
};
...
extern void scrm_write_mod_reg(u32 val, u16 idx);
...
static void __init omap4_init_voltagecontroller(void)
{
	...
	voltage_write_reg(OMAP4_PRM_VC_CFG_I2C_CLK_OFFSET,
		(0x11 << OMAP4430_HSSCLL_SHIFT |
		0x00 << OMAP4430_HSSCLH_SHIFT |
		0x3B << OMAP4430_SCLL_SHIFT |
		0x26 << OMAP4430_SCLH_SHIFT));

#if 1
	voltage_write_reg(OMAP4_PRM_VC_CFG_I2C_MODE_OFFSET,
		0x8 << OMAP4430_HSMCODE_SHIFT | 0x1 << OMAP4430_DFILTEREN_SHIFT);
#else
    /* Disable HS mode & keep start-repeat operation enabled*/
    voltage_write_reg(OMAP4_PRM_VC_CFG_I2C_MODE_OFFSET,
        voltage_read_reg(OMAP4_PRM_VC_CFG_I2C_MODE_OFFSET) & 0xFFFFFFF7);
#endif
	...
	voltage_write_reg(OMAP4_PRM_VOLTSETUP_CORE_OFF_OFFSET,
		(0x3 << OMAP4430_RAMP_DOWN_PRESCAL_SHIFT) |
		(0x3 << OMAP4430_RAMP_UP_PRESCAL_SHIFT) |
		(0xA << OMAP4430_RAMP_DOWN_COUNT_SHIFT) |
		(0xA << OMAP4430_RAMP_UP_COUNT_SHIFT));
	...
	prm_write_mod_reg(0x33F, OMAP4430_PRM_DEVICE_MOD, OMAP4_PRM_VOLTSETUP_WARMRESET_OFFSET);
	omap2_scrm_write_reg(0x3A0140, OMAP4_SCRM_CLKSETUPTIME_OFFSET);
	omap2_scrm_write_reg(0x3F003F, OMAP4_SCRM_PMICSETUPTIME_OFFSET);
}
...
static void __init vdd_data_configure(struct omap_vdd_info *vdd)
{
	...
	(void) debugfs_create_file("vp_errorgain", 0660,
				vdd_debug,
				&(vdd->vp_reg.vpconfig_errorgain),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_smpswaittimemin", 0660,
				vdd_debug,
				&(vdd->vp_reg.vstepmin_smpswaittimemin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_stepmin", 0660, vdd_debug,
				&(vdd->vp_reg.vstepmin_stepmin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_smpswaittimemax", 0660,
				vdd_debug,
				&(vdd->vp_reg.vstepmax_smpswaittimemax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_stepmax", 0660, vdd_debug,
				&(vdd->vp_reg.vstepmax_stepmax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_vddmax", 0660, vdd_debug,
				&(vdd->vp_reg.vlimitto_vddmax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_vddmin", 0660, vdd_debug,
				&(vdd->vp_reg.vlimitto_vddmin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_timeout", 0660, vdd_debug,
				&(vdd->vp_reg.vlimitto_timeout),
				&vp_debug_fops);
	...
	if (cpu_is_omap44xx() && !strcmp("vdd_iva", name))
		(void) debugfs_create_u8("fbb_enable", 0660,
...
static int __init omap_voltage_init(void)
{
	...
	/*SRAM Setting Time*/
	if (cpu_is_omap44xx()) {
		prm_write_mod_reg(0x784D9017,
				OMAP4430_PRM_DEVICE_MOD, OMAP4_PRM_SRAM_COUNT_OFFSET);
	}

#if 0
	{
		unsigned int volatile val = 0;

		/*sram mpu LDO ret software control */
		val = omap_readl(0x4A002324);	
		val = val & 0xFBE0FFFF;
		val = val | 0x04010000;
		omap_writel(val , 0x4A002324);

		/*sram core LDO ret software control */
		val = omap_readl(0x4A002328);	
		val = val & 0xFBE0FFFF;
		val = val | 0x04010000;
		omap_writel(val , 0x4A002328);

		/*sram iva LDO ret software control */
		val = omap_readl(0x4A002320);	
		val = val & 0xFBE0FFFF;
		val = val | 0x04010000;
		omap_writel(val , 0x4A002320);
	}
#endif
