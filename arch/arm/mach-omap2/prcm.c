...
static unsigned int g_reset_sp, g_reset_lr;
...
void omap2_scrm_write_reg(u32 val, u16 idx)
{

	__omap_prcm_write(val, scrm_base, 0x0, idx);
...
void __init omap2_set_globals_prcm(struct omap_globals *omap2_globals)
{
	...
	if (omap2_globals->scrm) {
		scrm_base = ioremap(omap2_globals->scrm, SZ_8K);
		WARN_ON(!scrm_base);
	}
}
