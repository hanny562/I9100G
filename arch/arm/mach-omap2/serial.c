...
extern spinlock_t  gps_uart_wa;
extern u32 gps_uart_wa_open_flag;
...
{
#ifdef GPS_UART_WORKAROUND
//		printk("omap_uart_block_sleep %d\n", uart->num);
#endif
...
#ifdef GPS_UART_WORKAROUND
//	printk("omap_uart_allow_sleep %d\n", uart->num);
#endif
...
static void omap_uart_idle_timer(unsigned long data)
{
	...
	if(uart->num == UART1 && gps_uart_wa_open_flag == GPS_UART_OPEN) //uart1 and already closed.
	{
//		printk("It's supposed to be close, but making it alive\n");
		omap_uart_block_sleep(uart);
	}
	else
	{
			...
			struct omap_uart_port_info *platform_data)
{
	...
	serial_write_reg(uart, UART_FCR,  UART_FCR_CLEAR_RCVR);
