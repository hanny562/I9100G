...
int hdmi_core_ddc_edid(u8 *pEDID, int ext)
{
	...
	#define HDMI_DDC_TIMEOUT 5000 /* DCC timeout in ms */
	...
	while (j < 128) {
		if (FLD_GET(hdmi_read_reg(ins, sts), 2, 2) == 0) {
			pEDID[i++] = hdmi_read_reg(ins, HDMI_CORE_DDC_DATA);
			j++;
			do_gettimeofday(&t1);
		}
		else {
			...
			if (FLD_GET(hdmi_read_reg(ins, sts), 5, 5) == 1) {
				/* I2C no ack... try again */
				printk(KERN_ERR "EDID I2C no ack - time=%d %d\n", time_elapsed_ms, j);
				...
				hdmi_read_reg(ins, HDMI_CORE_DDC_CMD);
			}

			if(time_elapsed_ms > HDMI_DDC_TIMEOUT) {
				printk(KERN_ERR "EDID timeout: no data during %d ms, status=%02x %d %u\n",
						HDMI_DDC_TIMEOUT,
	...
	while (hdmi_read_reg(ins, sts) != 0x04) {
		do_gettimeofday(&t2);
		time_elapsed_ms = (t2.tv_sec - t1.tv_sec) * 1000 + (t2.tv_usec - t1.tv_usec) / 1000;

		if(time_elapsed_ms > HDMI_DDC_TIMEOUT) {
			printk(KERN_ERR "EDID timeout: FIFO not getting empty - status=%02x\n",
						hdmi_read_reg(ins, sts));
			return -1;
		}
	}
...
#endif
	printk(KERN_ERR "EDID transfer done %02x %d", hdmi_read_reg(ins, sts), j);
