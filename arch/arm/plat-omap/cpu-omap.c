...
static struct cpufreq_frequency_table *cpu_per_freq_table[CONFIG_NR_CPUS]={0,};
...
{
	struct cpufreq_frequency_table *freq_table =cpu_per_freq_table[policy->cpu];
		       ...
		       unsigned int relation)
{
	...
	opp_find_freq_ceil(mpu_dev, &freq);
	if ( freq > 1008000000 && freqs.old*1000 < 1008000000 )
		target_freq = 1008000;

	freqs.new = clk_round_rate(mpu_clk, target_freq * 1000) / 1000; //target_freq*1000;
