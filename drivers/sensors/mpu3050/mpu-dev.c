...
static int mpu3050_factory_on(struct i2c_client *client)
{
	...
	if (1/*prev_gyro_suspended*/) {
		...
		printk("%s prev_gyro_suspended ",__func__);
...
static void mpu3050_factory_off(struct i2c_client *client)
{
	...
	if (1/*!mpu->mldl_cfg.gyro_is_suspended*/) {
		printk("%s: suspending on event \n", __func__);
		(void) mpu3050_suspend(mldl_cfg, this_client->adapter,
					accel_adapter, compass_adapter,
					pressure_adapter, TRUE, TRUE, TRUE, TRUE);
			...
			short *temp_avg)
{
    ...
    for (j = 0; j < 3; j++) {
        if (abs(Avg[j]) > (int)DEF_BIAS_THRESH) {
            printk("\n %s-Gyro bias (%.0d) exceeded threshold (threshold = %f)\n",
...
{
	unsigned char mpu_name[16]="mpu3050";
	struct regulator *reg_vaux1;
	struct regulator *reg_vusim;
	...
	if (hw_rev==1) {
		printk(" @@@ sensor_ldo_reset for Q1 rev01 :%d@@@ \n", hw_rev);
		this_client->dev.init_name = mpu_name;
		reg_vaux1 = regulator_get(&this_client->dev, "vaux2");
		if (IS_ERR(reg_vaux1)) {
			printk("[mpu] %s [%d] failed to get vaux2 regulator. \n",
			       __func__, __LINE__);
			return;
		}

		reg_vusim = regulator_get(&this_client->dev, "vusim");
		if (IS_ERR(reg_vusim)) {
			printk("[mpu] %s [%d] failed to get vusim regulator. \n",
			       __func__, __LINE__);
			return;
		}

		regulator_enable(reg_vusim);
		regulator_disable(reg_vusim);

		regulator_enable(reg_vaux1);
		regulator_disable(reg_vaux1);
		...
		regulator_put(reg_vaux1);
		regulator_put(reg_vusim);
	}
}
#endif
