...
/* Slave addresses */
#define MXT224_APP_LOW		0x4C
#define MXT224_APP_HIGH		0x4D
#define MXT224_BOOT_LOW		0x26
#define MXT224_BOOT_HIGH		0x27

/* FIRMWARE NAME */
#define MXT224_ECHO_FW_NAME	    "mXT540E.fw"
#define MXT224_FW_NAME		    "mXT224.fw"

#define MXT224_FWRESET_TIME		500	/* msec */
#define MXT224_RESET_TIME		300	/* msec */
...
/*	u16 obj_address = 0;
	...
	if (data->family_id == 0xA1 && panel_id >= 0x33) {
		get_object_info(data, PROCG_NOISESUPPRESSION_T48, &size, &obj_address);
		if (ta_status) {
			value = data->calcfg_charging_e;
...
static int __devinit mxt224_init_touch_driver(struct mxt224_data *data)
{
	...
	dev_info(&data->client->dev, "family = %#02x, variant = %#02x, version "
			"= %#02x, build = %d, panel_id = %#02x\n", id[0], id[1], id[2], id[3], panel_id);
...
{
	int i;
	if (data->family_id == 0xA1 && panel_id >= 0x33) {	/*  : MXT-224E */
...
static void mxt224_late_resume(struct early_suspend *h)
{
	...
	if (data->read_ta_status) {
		...
		if (!(deepsleep && ta_status)) mxt224_ta_probe(ta_status);
	}
...
static u8 firmware_latest[] ={0x02, 0x10}; /* mxt224 : 0x16, mxt224E : 0x10 */
static u8 build_latest[] = {0x00, 0x02};
...
static ssize_t set_refer0_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(data->family_id == 0x81) mxt_reference -= 16384;
...
static ssize_t set_mxt_firm_update_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	else if (data->family_id == 0xA1 && panel_id >= 0x33 )  {	/* tsp_family_id - 0x81 : MXT-224E */
...
static ssize_t set_mxt_firm_version_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	else if (data->family_id == 0xA1 && panel_id >= 0x33) fw_latest_version = firmware_latest[1];
...
static int __devinit mxt224_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (data->family_id == 0xA1 && panel_id == 0x23) {	/*  : MXT-540E */
		...
		printk("[TSP] TSP chip is MXT540-B\n");
	}
	else if (data->family_id == 0xA1 && panel_id >= 0x33)  {	/* tsp_family_id - 0x81 : MXT-224E */
		...
		printk("[TSP] TSP chip is MXT540-E\n");
	...
	if (IS_ERR(mxt224_noise_test))
		printk(KERN_ERR "Failed to create device(mxt224_noise_test)!\n");
