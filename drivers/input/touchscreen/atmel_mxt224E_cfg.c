...
uint8_t mxt_touch_threshold = T9_TCHTHR_BATT;
...
int mxt_get_object_values(struct mxt_data *mxt, int obj_type)
{
	...
	case MXT_USER_INFO_T38:
		...
		break;
	default:
		pr_err("Not supporting object type (object type: %d)", obj_type);
		return -1;
	}
...
int mxt_multitouch_config(struct mxt_data *mxt)
{
	...
	if (mxt->set_mode_for_ta) touchscreen_config.tchthr       = T9_TCHTHR_TA;
	else touchscreen_config.tchthr       = T9_TCHTHR_BATT;
...
int mxt_noisesuppression_t48_config(struct mxt_data *mxt)
{
	...
	if ((mxt->device_info.major <= 0x00) && (mxt->device_info.minor <= 0x04) && (mxt->device_info.build <= 0x0e)) {
		noisesuppression_t48_config.ctrl                 = 1;  
	 	noisesuppression_t48_config.cfg                  = 0; 
	 	noisesuppression_t48_config.calcfg               = 40; 
	 	noisesuppression_t48_config.basefreq             = 0;  
	 	noisesuppression_t48_config.freq_0               = 0;  
	 	noisesuppression_t48_config.freq_1               = 0;  
	 	noisesuppression_t48_config.freq_2               = 0;  
	 	noisesuppression_t48_config.freq_3               = 0;  
	 	noisesuppression_t48_config.mffreq_2             = 0;  
	 	noisesuppression_t48_config.mffreq_3             = 0;  
	 	noisesuppression_t48_config.nlgain               = 0;  
	 	noisesuppression_t48_config.nlthr                = 60; 
	 	noisesuppression_t48_config.gclimit              = 31; 
	 	noisesuppression_t48_config.gcactvinvldadcs      = 6;  
	 	noisesuppression_t48_config.gcidleinvldadcs      = 50; 
	 	noisesuppression_t48_config.gcinvalidthr         = 64; 
	 	noisesuppression_t48_config.gcmaxadcsperx        = 100;
	}
	else if ((mxt->device_info.major == 0x00) && (mxt->device_info.minor == 0x05) && (mxt->device_info.build == 0x07)) {
...
recheck:
	...
	case WAITING_FRAME_DATA:
		...
		if (val == APP_CRC_FAIL) {
			pr_info("We've got a APP_CRC_FAIL, so try again (count=%d)", ++retry);
			goto recheck;
		}
		break;
...
int mxt_load_firmware(struct device *dev, const char *fn)
{
	...
	if (mxt->client->addr == MXT_I2C_APP_ADDR) {
		pr_info("I2C address: 0x%02X --> 0x%02X", MXT_I2C_APP_ADDR, MXT_I2C_BOOTLOADER_ADDR);
	...
	pr_info("Firmware info: version [0x%02X], build [0x%02X]", fw->data[0], fw->data[1]);
...
try_to_resend_the_last_frame:
		...
		if (ret < 0) {
			if (++retry < 10) {
				...
				pr_info("We've got a FRAME_CRC_FAIL, so try again up to 10 times (count=%d)", retry);
...
err_fw:
	...
	if (mxt->client->addr == MXT_I2C_BOOTLOADER_ADDR) {
		pr_info("I2C address: 0x%02X --> 0x%02X", MXT_I2C_BOOTLOADER_ADDR, MXT_I2C_APP_ADDR);
