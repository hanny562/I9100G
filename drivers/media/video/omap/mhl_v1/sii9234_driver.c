...
#define MHL_RESET_TIMER
#ifdef MHL_RESET_TIMER
...
#define SII_DRV_DBG(format,...)\
	printk ("[ MHL_DRV(%s) ] " format "\n", __func__, ## __VA_ARGS__)
...
extern void ocp_interrupt_clear(void);
...
bool vbus_mhl_est_fail;
bool mhl_audio_path_info=FALSE;
//
...
extern void FSA9480_CheckAndHookAudioDock(void);//smpark

#ifdef MHL_RESET_TIMER
static void mhl_reset_timer_proc(unsigned long ptr);
static void add_mhl_reset_timer(void);
static void del_mhl_reset_timer(void);
static void mhl_reset_timer_run(struct work_struct *data);

static struct timer_list s_mhl_reset_timer;
static int b_mhl_reset_timer_init = 0;
static DECLARE_WORK(mhl_rst_wq, mhl_reset_timer_run);
...
static void mhl_reset_timer_proc(unsigned long ptr)
{
	del_timer(&s_mhl_reset_timer);
	SII_DRV_DBG("timer expired\n");
	schedule_work(&mhl_rst_wq);
}

static void add_mhl_reset_timer(void)
{
	if(b_mhl_reset_timer_init)
	{
		del_timer(&s_mhl_reset_timer);
		b_mhl_reset_timer_init = 0;
	}
	SII_DRV_DBG("timer started\n");
	init_timer(&s_mhl_reset_timer);

	s_mhl_reset_timer.function = mhl_reset_timer_proc;
	s_mhl_reset_timer.data = 0;
	s_mhl_reset_timer.expires = jiffies + HZ * 8;    /* 8 seconds */
	add_timer(&s_mhl_reset_timer);
	b_mhl_reset_timer_init = 1;
}

static void del_mhl_reset_timer(void)
{
	SII_DRV_DBG("timer_del %d\n",b_mhl_reset_timer_init);
	if(b_mhl_reset_timer_init == 1)
	{
		del_timer(&s_mhl_reset_timer);
		b_mhl_reset_timer_init = 0;
	}
...
Bool SiI9234_startTPI(void)
{
	...
	{
		SII_DRV_DBG("######## Subhransu: Silicon Device Id: %x\n", devID);
		return TRUE;
	}

	SII_DRV_DBG( "Unsupported TX\n");
	return FALSE;
}
...
Bool sii9234_init(void)
{
	SII_DRV_DBG("# SiI9234 initialization start~ \n");
...
{
	SII_DRV_DBG ("Drv: SiiMhlTxChipInitialize: %02X44\n", (int)I2C_ReadByte(SA_TX_Page0_Primary, 0x03));
...
void	SiiMhlTxDrvTmdsControl( bool enable )
{
	...
	{
		SET_BIT(SA_TX_Page0_Primary, 0x80, 4);
	    	SII_DRV_DBG("Drv: TMDS Output Enabled\n");
//3355
...
//3355
	...
	{
		CLR_BIT(SA_TX_Page0_Primary, 0x80, 4);
		SII_DRV_DBG("Drv: TMDS Ouput Disabled\n");
	}
}
///////////////////////////////////////////////////////////////////////////////
//
...
{
	SII_DRV_DBG("Drv: SiiMhlTxDrvNotifyEdidChange\n");
	//
...
bool SiiMhlTxDrvSendCbusCommand ( cbus_req_t *pReq  )
{
  ...
  if( (TX_POWER_STATE_D0_MHL != fwPowerState ) || (mscCmdInProgress))
	{
	    SII_DRV_DBG("Error: Drv: fwPowerState: %02X, CBUS[0x0A]: %02X or mscCmdInProgress = %d\n",
				(int) fwPowerState,
	...
	SII_DRV_DBG("Drv: Sending MSC command %02X, %02X, %02X, %02X\n",
				(int)pReq->command,
				(int)(pReq->offsetData),
				(int)pReq->msgData[0],
				(int)pReq->msgData[1]);
...
//		0 = RSVD		(reserved)
////////////////////////////////////////////////////////////////////
extern void hdmi_change_status();
...
void	Int1RsenIsr( void )
{
	...
	SII_DRV_DBG("Drv: Got INTR_1: reg71 = %02X, rsen = %02X\n", (int) reg71, (int) rsen);
	if(0xFF == reg71 || 0xFA == reg71)
	{
		SII_DRV_DBG( "RETURN (reg71 == 0x%X)\n", reg71);
		return;
	}
	if(reg71 & BIT_5)
	{
		...
		if(rsen == 0x00)
		{
	      		if(TX_POWER_STATE_D0_MHL != fwPowerState)
	      		{
				SII_DRV_DBG("Drv: Got1 INTR_1: reg71 = %02X, rsen = %02X\n", (int) reg71, (int) rsen);
...
#if 1
      			{
        			SII_DRV_DBG(KERN_ERR "RSEN Low and 1K impedance\n");
			//	DelayMS(120);
				DelayMS(200);
					...
					SII_DRV_DBG(KERN_ERR "Really RSEN Low\n");
					//mhl_cable_status =MHL_INIT_POWER_OFF;
				#if 0
					u32 temp = omap_readl(0x48046030);
                			SII_DRV_DBG("WP_IRQ_ENABLE_CLR read as %x\n",temp);
                			temp =(temp | (1<<4));
                			omap_writel(temp,0x48046030);
                			SII_DRV_DBG("Cleared OCP_timeout if any..WP_IRQ_ENABLE_CLR read as %x\n",temp);
          			#endif
	...
	FSA9480_MhlSwitchSel(0);
#endif
					dsHpdStatus = 0;
					phy_control_interrupt(0);
					MHL_On(0);
					//ocp_interrupt_clear();
         			}
        		 	else
        			{
         				SII_DRV_DBG(KERN_ERR "RSEN Stable\n");
        			}
			...
			{
			mhl_cable_status =MHL_TV_OFF_CABLE_CONNECT;
			SII_DRV_DBG("MHL Connection Fail Power off ###\n");
			SiI9234_HW_Reset();
			For_check_resen_int();
			...
			return ;
	      		...
	      		{
				SII_DRV_DBG("%s: MHL Cable disconnect### 2\n", __func__);
				mhl_cable_status =MHL_INIT_POWER_OFF;
#ifdef MHL_RESET_TIMER
				del_mhl_reset_timer();
#endif
		...
		else if(rsen == 0x04)
		{
			...
			SII_DRV_DBG("MHL Cable Connection ###\n");
		}
...
#else
			if (BIT_6 & cbusInt){
				 SII_DRV_DBG("Drv: Downstream HPD HIGH\n"); 
				MASK_CBUS1_INTERRUPTS;
				MASK_CBUS2_INTERRUPTS;
				ReadModifyWriteTPI(0x79, BIT_5 | BIT_4,  BIT_5 | BIT_4); //hpd make it high

			}
			else{

				 SII_DRV_DBG("Drv: Downstream HPD LOW\n"); 
				CbusReset();

				//sangmi
				InitCBusRegs();

				SII_DRV_DBG("Drv: hpd make it low\n");
				ReadModifyWriteTPI(0x79, BIT_5 | BIT_4, BIT_4); //hpd make it low
...
void WriteInitialRegisterValues ( void )
{
	SII_DRV_DBG("Drv: WriteInitialRegisterValues\n");
	// Power Up
	...
	I2C_WriteByte(SA_TX_Page0_Primary, 0x91, 0xA7);		// Clear bit 6 (reg_skip_rgnd)


	// Changed from 66 to 65 for 94[1:0] = 01 = 5k reg_cbusmhl_pup_sel
	//I2C_WriteByte(SA_TX_Page0_Primary, 0x94, 0x65);			// 1.8V CBUS VTH & GND threshold
    I2C_WriteByte(SA_TX_Page0_Primary, 0x94, 0x45);			// 1.8V CBUS VTH & GND threshold
    ...
    I2C_WriteByte(SA_TX_Page0_Primary, 0xA5, 0xB0);
	SII_DRV_DBG("Drv: MHL 1.0 Compliant Clock\n");
...
static void InitCBusRegs( void )
{
	...
	SII_DRV_DBG("Drv: InitCBusRegs\n");
	// Increase DDC translation layer timer
	I2C_WriteByte(SA_TX_CBUS_Primary, 0x07, 0x32);
...
{
	SII_DRV_DBG("Drv: CbusWakeUpPulseGenerator\n");
...
static	void	ApplyDdcAbortSafety()
{
	...
	if ((bPost > (bTemp + 50)))
	{
		SII_DRV_DBG("Drv: Applying DDC Abort Safety(SWWA 18958)\n");
...
void	ProcessRgnd( void )
{
	...
	SII_DRV_DBG("Drv: RGND Reg 99 = %02X : ", (int)reg99RGNDRange);
#if defined(CONFIG_S5PC110_DEMPSEY_BOARD)
	...
	if (reg99RGNDRange == 0x00 || reg99RGNDRange == 0x03)
	{
		...
		SII_DRV_DBG(" : USB impedance. Disable MHL discovery.\n", (int)reg99RGNDRange);

		CLR_BIT(SA_TX_Page0_Primary, 0x95, 5);
		mhl_cable_status =MHL_INIT_POWER_OFF;
		SII_DRV_DBG(KERN_ERR "MHL_SEL to 0\n");
		SII_DRV_DBG(KERN_ERR "CALL AUDIO DOCK\n");

		mhl_audio_path_info = TRUE;
#ifdef MHL_RESET_TIMER
		del_mhl_reset_timer();
#endif
...
static void For_check_resen_int (void)
{
	...
	I2C_WriteByte(SA_TX_Page0_Primary, 0x91, 0xA7);
	...
	I2C_WriteByte(SA_TX_Page0_Primary, 0xA5, 0xB0);			// RGND Hysterisis.
...
//	I2C_WriteByte(SA_TX_Page0_Primary, 0x97, 0x03);			// Auto Heartbeat failure enable
  ...
  I2C_WriteByte(SA_TX_CBUS_Primary, 0x07, DDC_XLTN_TIMEOUT_MAX_VAL | 0x2); 			// Increase DDC translation layer timer
...
static	void	Int4Isr( void )
{
	...
	SII_DRV_DBG( "[MHL] Int4Isr : REG74 : %x ,mhl_cable_status=%d, mhl_vbus=%d\n",(int)reg74,(int)mhl_cable_status,(int)mhl_vbus);
	// When I2C is inoperational (say in D3) and a previous interrupt brought us here, do nothing.
	if(0xFF == reg74 || 0xFA == reg74)
	{
		SII_DRV_DBG("RETURN (reg74 == 0x%X)\n",reg74);
		return;
	}
	...
	{
		if((I2C_ReadByte(SA_TX_Page0_Primary, 0x09) & BIT_2) == 0x00) {
			SII_DRV_DBG( "[MHL] RSEN is low - status incorrect\n");
#ifdef MHL_RESET_TIMER
			del_mhl_reset_timer();
#endif
			MHL_On(0);
...
void MhlTxDrvProcessConnection ( void )
{
	...
	SII_DRV_DBG("Drv: MHL Cable Connected. CBUS:0x0A = %02X\n", (int) ReadByteCBUS(0x0a));

	if( TX_POWER_STATE_D0_MHL == fwPowerState )
	{
		SII_DRV_DBG("POWER_STATE_D0_MHL == fwPowerState\n");
		return;
	}
...
void MhlTxDrvProcessDisconnection ( void )
{
	...
	SII_DRV_DBG("Drv: MhlTxDrvProcessDisconnection\n");
...
static byte CBusProcessErrors( byte intStatus )
{
	...
	if ( intStatus )
	{
		...
		if( intStatus & BIT_DDC_ABORT )
		{
			...
			SII_DRV_DBG ("CBUS DDC ABORT happened, reason:: %02X\n", (int)(ddcAbortReason));
		}
		...
		if ( intStatus & BIT_MSC_XFR_ABORT )
		{
			...
			SII_DRV_DBG ("CBUS:: MSC Transfer ABORTED. Clearing 0x0D\n");
			WriteByteCBUS((byte) REG_PRI_XFR_ABORT_REASON, 0xFF );
		}
		if ( intStatus & BIT_MSC_ABORT )
		{
			SII_DRV_DBG ("CBUS:: MSC Peer sent an ABORT. Clearing 0x0E\n");
		...
		if ( mscAbortReason != 0 )
		{
			SII_DRV_DBG ("CBUS:: Reason for ABORT is ....0x%02X = ", (int)mscAbortReason );

			if ( mscAbortReason & CBUSABORT_BIT_REQ_MAXFAIL)
			{
				SII_DRV_DBG ("Requestor MAXFAIL - retry threshold exceeded\n");
			}
			if ( mscAbortReason & CBUSABORT_BIT_PROTOCOL_ERROR)
			{
				SII_DRV_DBG ("Protocol Error\n");
			}
			if ( mscAbortReason & CBUSABORT_BIT_REQ_TIMEOUT)
			{
				SII_DRV_DBG ("Requestor translation layer timeout\n");
			}
			if ( mscAbortReason & CBUSABORT_BIT_PEER_ABORTED)
			{
				SII_DRV_DBG ("Peer sent an abort\n");
			}
			if ( mscAbortReason & CBUSABORT_BIT_UNDEFINED_OPCODE)
			{
				SII_DRV_DBG ("Undefined opcode\n");
			}
		}
	}
	return( result );
...
static void MhlCbusIsr( void )
{
	...
	if((cbusInt & BIT_3))
	{
		SII_DRV_DBG("Drv: MSC_MSG Received: %02X\n", (int) cbusInt);
		//
	...
	if(cbusInt & BIT_4)
	{
		SII_DRV_DBG("Drv: MSC_REQ_DONE: %02X\n", (int) cbusInt);
	...
	if(cbusInt)
	{
		...
		SII_DRV_DBG("Drv: Clear CBUS INTR_1: %02X\n", (int) cbusInt);
	}
	//
	// Clear all interrupts that were raised even if we did not process
	//
	...
	if( cbusInt )
	{
		SII_DRV_DBG("Drv: CBUS INTR_2: %x\n", (int) cbusInt);
	}
	if(cbusInt & BIT_2)
	{
		SII_DRV_DBG("Drv: INT Received: %x\n", (int) cbusInt);
	...
	if(cbusInt & BIT_3)
	{
		SII_DRV_DBG("Drv: STATUS Received: %x\n", (int) cbusInt);
	...
	if(cbusInt)
	{
		...
		SII_DRV_DBG("Drv: Clear CBUS INTR_2: %02X\n", (int) cbusInt);
	}
...
void sii_mhl_tx_init( void )
{
	SII_DRV_DBG ("MhlTx: sii_mhl_tx_init\n");
...
void SiiMhlTxGetEvents( byte *event, byte *eventParameter )
{
	...
	*eventParameter = 0;
	...
	{
		SII_DRV_DBG ("MhlTx: SiiMhlTxGetEvents mhlConnectionEvent\n");
	...
	{
		SII_DRV_DBG("MhlTx: SiiMhlTxGetEvents MSC MSG Arrived\n");
...
/* jinho96.kim 2011.01.06 */
				...
				SII_DRV_DBG("Key Code:%x \n",(int)mhlTxConfig.mscMsgData);
                                }
else
				{
  				SII_DRV_DBG("Key Code Error:%x \n",(int)mhlTxConfig.mscMsgData);					SiiMhlTxRcpeSend( 0x01 );
				}
				break;
...
static	void	MhlTxDriveStates( void )
{
	...
	switch( mhlTxConfig.mscState )
	{
		...
		case MSC_STATE_POW_DONE:
			...
			//
			SII_DRV_DBG( "MSC_STATE_POW_DONE \n");
...
void	SiiMhlTxMscCommandDone( byte data1 )
{
	SII_DRV_DBG ("MhlTx: SiiMhlTxMscCommandDone. data1 = %02X\n", (int) data1);
			...
			(MHL_DEV_FEATURE_FLAG_OFFSET == mhlTxConfig.mscLastOffset))
	{
		...
		SII_DRV_DBG ("MhlTx: Peer's Feature Flag = %02X\n\n", (int) data1);
	}
...
void	SiiMhlTxGotMhlIntr( byte intr_0, byte intr_1 )
{
	SII_DRV_DBG ("MhlTx: INTERRUPT Arrived. %02X, %02X\n", (int) intr_0, (int) intr_1);
...
void	SiiMhlTxGotMhlStatus( byte status_0, byte status_1 )
{
	SII_DRV_DBG ("MhlTx: STATUS Arrived. %02X, %02X\n", (int) status_0, (int) status_1);
	//
...
void	SiiMhlTxNotifyDsHpdChange( byte dsHpdStatus )
{
	if( 0 == dsHpdStatus )
	{
		//SII_DRV_DBG("MhlTx: Disable TMDS\n");
		//SiiMhlTxDrvTmdsControl( FALSE );
	}
	else
	{
		SII_DRV_DBG("MhlTx: Enable TMDS\n");
#ifdef MHL_RESET_TIMER
		del_mhl_reset_timer();
#endif
		SiiMhlTxDrvTmdsControl( TRUE );
...
void	AppRcpDemo( byte event, byte eventParameter)
{
	...
	switch( event )
	{
		case	MHL_TX_EVENT_DISCONNECTION:
			SII_DRV_DBG("App: Got event = MHL_TX_EVENT_DISCONNECTION\n");
			break;

		case	MHL_TX_EVENT_CONNECTION:
			SII_DRV_DBG("App: Got event = MHL_TX_EVENT_CONNECTION\n");
			break;
		...
		case	MHL_TX_EVENT_RCP_READY:
			...
			SII_DRV_DBG( "App: Got event = MHL_TX_EVENT_RCP_READY...Sending RCP (%02X)\n", (int) rcpKeyCode);
		...
		case	MHL_TX_EVENT_RCP_RECEIVED:
			...
			//
			SII_DRV_DBG( "App: Received an RCP key code = %02X\n", eventParameter );
...
rcpKeyCode = eventParameter;
		...
		case	MHL_TX_EVENT_RCPK_RECEIVED:
			SII_DRV_DBG( "App: Received an RCPK = \n");
			break;

		case	MHL_TX_EVENT_RCPE_RECEIVED:
			SII_DRV_DBG( "App: Received an RCPE = \n");
			break;
...
void	AppVbusControl( bool powerOn )
{
	if( powerOn )
	{
		SII_DRV_DBG( "App: Peer's POW bit is set. Turn the VBUS power OFF here.\n");
	}
	else
	{
		SII_DRV_DBG( "App: Peer's POW bit is cleared. Turn the VBUS power ON here.\n");
	}
}
...
void sii9234_interrupt_event(void)
{
	...
	do {
		...
		flag = 0;
		val = I2C_ReadByte(SA_TX_Page0_Primary, (0x74));        // read status

               // When I2C is inoperational (say in D3) and a previous interrupt brought us here, do nothing.
               if(0xFF == val || 0xFA == val)
               {
                       SII_DRV_DBG("%s [%d] (reg74 == 0x%X)\n",__func__, __LINE__, val);
                       return;
               }
    	...
    	if(mhl_cable_status == MHL_TV_OFF_CABLE_CONNECT)
		{
    		...
    		tmp = ReadByteCBUS(0x1E);
    		WriteByteCBUS(0x1E, tmp);
    		SII_DRV_DBG("mhl_cable_status == MHL_TV_OFF_CABLE_CONNECT\n");
    		return ;
        ...
        mhl_cable_status) //NAGSM_Android_SEL_Kernel_Aakash_20101214
      {
          ...
          SII_DRV_DBG("#1 (0x71) flag: %x\n",(int) flag );

          if(mhlTxConfig.mhlConnected == MHL_TX_EVENT_DISCONNECTION)//(mhlTxConfig_status()== MHL_TX_EVENT_DISCONNECTION)//
          {
            tmp = ReadByteCBUS(0x08);
            SII_DRV_DBG("#2 (ReadByteCBUS(0x08)) Temp: %x\n",(int) tmp );
            WriteByteCBUS(0x08, tmp);

            tmp = ReadByteCBUS(0x1E);
            SII_DRV_DBG("#2 (ReadByteCBUS(0x1E)) Temp: %x\n",(int) tmp );
            WriteByteCBUS(0x1E, tmp);
          ...
          else
          {
            ...
            SII_DRV_DBG("#1 (ReadByteCBUS(0x08)) Temp: %x\n",(int) flag );

			tmp = ReadByteCBUS(0x1E);
            flag |= (tmp&INTR_CBUS2_DESIRED_MASK);
            SII_DRV_DBG("#1 (ReadByteCBUS(0x1E)) Temp: %x\n",(int) flag );
          }
...
void PrintMhlReg(void)
{
	...
	SII_DRV_DBG ("//======= 0x72 REG dump\n");
	for ( i=0; i<255;i++) {

		a = I2C_ReadByte(0x72, i);
	...
	printk ("SA_TX_CBUS_Primary, 0x7, 0x32\n");

	I2C_WriteByte(SA_TX_CBUS_Primary, 0x7, 0x32);


}
EXPORT_SYMBOL(PrintMhlReg);
