...
struct switch_dev *usb_switch_sio_dev;
struct switch_dev *uart_switch_sio_dev;
...
static struct switch_sio_platform_data *switch_sio_pdata;
...
{
    /*init gpio*/
    switch_sio_pdata->gpio_init();	
	switch_sio_pdata->cp_usb_cb(USB_SW_AP);

	/* request by HW : after cp on then uart selection should be Modem */
	//switch_sio_pdata->uart_sel_cb(UART_SW_CP);
	switch_sio_pdata->uart_sel_cb(UART_SW_AP);
}
...
{	
	...
	if(usb_switch == USB_SW_CP)
	{
		...
		switch_sio_pdata->cp_usb_cb(USB_SW_CP);
#ifdef CONFIG_FSA9480_MICROUSB
        strncpy(buf,"AUDIO",5);
...
/* for sysfs control (/sys/devices/platform/switch-sio/usb_sel) */
...
/* for sysfs control (/sys/devices/platform/switch-sio/uart_sel) */
...
#else
      /*set default value with AP USB/UART*/
      switch_sel = USB_SEL_MASK | UART_SEL_MASK;
#endif
...
{
	int ret = 0;
	switch_sio_pdata = pdev->dev.platform_data;
