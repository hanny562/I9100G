...
#define GPS_UART_WORKAROUND

#ifdef GPS_UART_WORKAROUND
#define GPS_UART_CLOSE		0
#define GPS_UART_OPEN		1

spinlock_t gps_uart_wa;
u32 gps_uart_wa_open_flag = GPS_UART_CLOSE;
#endif //GPS_UART_WORKAROUND
...
static int serial_omap_startup(struct uart_port *port)
{
	...
	int retval;
		...
		if((up->pdev->id) == UART1)
		{
			omap_mux_init_signal("mcspi1_cs1", OMAP_MUX_MODE1 | OMAP_PIN_INPUT);
		}
#endif 
	...
	serial_out(up, UART_IER, up->ier);
		...
		if((up->pdev->id) == UART1)
		{
//			printk("UART1 channel open from OMAP\n");
			spin_lock(&gps_uart_wa);
			gps_uart_wa_open_flag = GPS_UART_OPEN;
			spin_unlock(&gps_uart_wa);
		}
#endif //
...
#ifdef GPS_UART_WORKAROUND
		if((up->pdev->id) == UART1) {
//			printk("UART1 channel close from OMAP\n");		
			spin_lock(&gps_uart_wa);
			gps_uart_wa_open_flag = GPS_UART_CLOSE;
			spin_unlock(&gps_uart_wa);
			omap_mux_init_signal("mcspi1_cs1", OMAP_MUX_MODE7 | OMAP_PIN_INPUT_PULLDOWN);

			serial_out(up, UART_FCR, UART_FCR_CLEAR_RCVR);
...
#if 0// TI patch --> UART Tx errata patch
