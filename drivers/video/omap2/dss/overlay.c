		...
		size_t size)
{
	...
	if (mgr && sysfs_streq(mgr->name, "tv")) {
		...
		if (mgr->device->panel.timings.x_res < info.out_height   ||
			mgr->device->panel.timings.y_res < info.out_width) {
			printk(KERN_ERR"//==== output window size exceeds panel dimensions");
			printk(KERN_ERR"mgr->device->panel.timings.x_res = %d", mgr->device->panel.timings.x_res);
			printk(KERN_ERR"info.out_width = %d", info.out_width);
			printk(KERN_ERR"mgr->device->panel.timings.y_res = %d", mgr->device->panel.timings.y_res);
			printk(KERN_ERR" info.out_height = %d",  info.out_height);
...
static u32 HDMI_detection_flag = 0;
void dss_recheck_connections(struct omap_dss_device *dssdev, bool force)
{
	...
	if (dssdev->type == OMAP_DISPLAY_TYPE_VENC
		...
		if(HDMI_detection_flag == 0)
		{
			printk("HDMI not detected - skip omap_dss_set_manager!\n");
			goto skip_path_setting;
		}
	}
