...
void dispc_setup_plane_fifo(enum omap_plane plane, u32 low, u32 high)
{
	...
	if (!dss_get_mainclk_state()) {
		DSSERR("mainclk disabled while trying"
			"dispc_setup_plane_fifo, returning\n");
		return;
	}
				...
				int three_tap)
{
	...
	return orig_size < out_size ? fir5_m8 :
		two_m < 17 ? fir5_m8b :
		two_m < 19 ? fir5_m9 :
		two_m < 21 ? fir5_m10 :
		two_m < 23 ? fir5_m11 :
				...
				struct omap_video_timings *timings)
{
	...
	DSSINFO("channel %u xres %u yres %u\n", channel, timings->x_res,
						timings->y_res);
	DSSINFO("pck %u\n", timings->pixel_clock);
	DSSINFO("hsw %d hfp %d hbp %d vsw %d vfp %d vbp %d\n",
			timings->hsw, timings->hfp, timings->hbp,
			timings->vsw, timings->vfp, timings->vbp);

	DSSINFO("hsync %luHz, vsync %luHz\n", ht, vt);
}
...
void dispc_irq_handler(void)
{
	...
	for (i = 0; i < DISPC_MAX_NR_ISRS; i++) {
		...
		if (isr_data->mask & irqstatus) {
			...
			{
				if (isr_data->mask & irqstatus & DISPC_IRQ_VSYNC2)
