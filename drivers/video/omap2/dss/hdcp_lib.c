...
static void hdcp_lib_generate_an(u8 *an)
{
	...
	DBG("AN: %x %x %x %x %x %x %x %x", an[0],an[1], an[2], an[3], an[4],an[5], an[6], an[7]);
}
...
void hdcp_lib_set_encryption(enum encryption_state enc_state)
{
	...
	printk("Encryption state changed: %s hdcp_ctrl: %02x",
...
int hdcp_lib_step2(void)
{
	...
	if (hdcp_sha1_result(&sha_n)) {
		DBG("SHA-1: ERROR - Could not compute message digest");
