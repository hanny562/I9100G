...
#ifdef CONFIG_OMAP2_DSS_DSI_VIDEOMODE_SUPPORT
...
static void dsi_complexio_config(struct omap_dss_device *dssdev)
{
	...
	/* modify for support data 4 lane */
...
#else
r = dsi_read_reg(ix, DSI_DSIPHY_CFG0);
r = FLD_MOD(r, 0x14, 31, 24);
r = FLD_MOD(r, 0x2E, 23, 16);
r = FLD_MOD(r, 0x14, 15, 8);
r = FLD_MOD(r, 0x25, 7, 0);
dsi_write_reg(ix, DSI_DSIPHY_CFG0, r);

r = dsi_read_reg(ix, DSI_DSIPHY_CFG1);
r = FLD_MOD(r, 0x7, 22, 16);
r = FLD_MOD(r, 0x11, 15, 8);
r = FLD_MOD(r, 0x43, 7, 0);
dsi_write_reg(ix, DSI_DSIPHY_CFG1, r);

r = dsi_read_reg(ix, DSI_DSIPHY_CFG2);
r = FLD_MOD(r, 0x10, 11, 0);
dsi_write_reg(ix, DSI_DSIPHY_CFG2, r);
#endif
}
...
int dsi_vc_send_bta_sync(enum omap_dsi_index ix, int channel)
{
	...
	if (r){
		DSSERR("Failed to SEND BTA Error !\n");
		goto err;
		}
...
int dsi_vc_dcs_write_in_interleaving(enum omap_dsi_index ix, int channel,
	u8 *data, int len)
{
	int r;
	int interleaved_max_time = 0xFFFF;

    /* youcef */
    /* clear Packet Sent interrupt */
    dsi_write_reg(ix, DSI_VC_IRQSTATUS(channel), DSI_VC_IRQ_PACKET_SENT );

	r = dsi_vc_dcs_write_nosync(ix, channel, data, len);
	if (r) {
		DSSERR("dsi_vc_dcs_write_nosync Error during interleaving!!!\n");
		goto err_interleaving;
		}

	while (REG_GET(ix, DSI_VC_IRQSTATUS(channel), 2, 2) != DSI_VC_IRQ_PACKET_SENT) {
		if (--interleaved_max_time == 0)
		goto err_interleaving;
	}
	...
	return 0;
err_interleaving:
	DSSERR("dsi_vc_dcs_write_in_interleaving(ch %d, cmd 0x%02x, len %d) failed during interleaving\n",
			channel, data[0], len);
	return r;
}
EXPORT_SYMBOL(dsi_vc_dcs_write_in_interleaving);
	...
	u8 *data, int len)
{
	...
	r = dsi_vc_dcs_write_nosync(ix, channel, data, len);
	if (r) {
		DSSERR("dsi_vc_dcs_write_nosync Error!!!\n");
		goto err;
		}
	...
	if (r)
		{
		DSSERR("dsi_vc_send_bta_sync Error!!!\n");
		goto err;
		}
	...
	u8 dcs_cmd, u8 *buf, int buflen)
{
	...
	if (r)
		goto err;
#else
	while(REG_GET(ix, DSI_VC_CTRL(channel) , 20, 20) != 0 )// read VC #1 FIFO not empty bit status
	{
		val = dsi_read_reg(ix, DSI_VC_SHORT_PACKET_HEADER(channel));
		DSSERR("\theader: %08x\n", val);
	}

	/* lets clear the VC #1 IBTA RQ STATUS*/
	REG_FLD_MOD(ix, DSI_VC_IRQSTATUS (channel), 1, 5, 5);

	/* lets perform a BTA ? this action give the hand of the Lane to Samsung*/
	REG_FLD_MOD(ix, DSI_VC_CTRL(channel), 1, 6, 6); /* BTA+EN = 1*/
	/* we have to wait until Samsung gives back the hand of the data lane ? you need to add a timeout mechanism here */
	while(REG_GET(ix, DSI_VC_IRQSTATUS (channel) , 5, 5) != 1 );
...
//		if (p_dsi->debug_read)
			DSSINFO("\tDCS short response, 1 byte: %02x\n", data);
...
//		if (p_dsi->debug_read)
			DSSINFO("\tDCS short response, 2 byte: %04x\n", data);
		...
		return 2;
	} else if (dt == DSI_DT_RX_DCS_LONG_READ || dt == 0x1a) {
		int w;
		int len = FLD_GET(val, 23, 8);
//		if (p_dsi->debug_read)
			DSSINFO("\tDCS long response, len %d\n", len);
...
//	r = FLD_MOD(r, 1, 9, 9); /* VP_DE_POL */
	...
	/* TL(31:16)=668,
	 * VACT(15:0)=1280 */
	dsi_write_reg(ix, DSI_VM_TIMING3, 0x03210500);
	/* HSA_HS_INTERLEAVING(23:16)=72,
	 * HFP_HS_INTERLEAVING(15:8)=114,
	 * HBP_HS_INTERLEAVING(7:0)=150 */
	dsi_write_reg(ix, DSI_VM_TIMING4, 0x00000000);
	/* HSA_LP_INTERLEAVING(23:16)=72,
	 * HFP_LP_INTERLEAVING(15:8)=114,
	 * HBP_LP_INTERLEAVING(7:0)=150 */
	dsi_write_reg(ix, DSI_VM_TIMING5, 0x00000000);
	/* BL_HS_INTERLEAVING(15:8)=49,
	 * BL_LP_INTERLEAVING(7:0)=209 */
	dsi_write_reg(ix, DSI_VM_TIMING6,(value_to_play_with <<16));

	/* ENTER_HS_MODE_LATENCY(23:16)=16,
	 * EXIT_HS_MODE_LATENCY(15:0)=14 */
	dsi_write_reg(ix, DSI_VM_TIMING7, 0x00120010);
	/* DDR_CLK_PRE(15:8), DDR_CLK_POST (7:0)*/	
	dsi_write_reg(ix, DSI_CLK_TIMING, 0x00001A0F);
	...
	/* set stop mode */
	REG_FLD_MOD(ix, DSI_TIMING1, 1, 15, 15); /*set bit at 1*/	
	while(REG_GET(ix, DSI_TIMING1, 15, 15) == 1  ); /* wait until bit is reset*/
