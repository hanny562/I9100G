	...
	if(cmc623_CABC_OnOff)
	{
		...
		switch(mode)
		{
			...
			case CMC623_DMB_MODE:
				lut_num = 0;
				cmc623_Mode_Change((mDNIe_data_type*)cmc623_values[CMC_DMB_CABC], TRUE, flag, lut_num);
			break;
	...
	else
	{
		...
		switch(mode)
		{
			...
			case CMC623_VIDEO_WARM_MODE:
				lut_num = 0;
				cabc_enable = 0;
				cmc623_Mode_Change((mDNIe_data_type*)cmc623_values[CMC_Video_CABC], FALSE, flag, lut_num);
			break;
			...
			case CMC623_DMB_MODE:
				lut_num = 0;
				cmc623_Mode_Change((mDNIe_data_type*)cmc623_values[CMC_DMB], FALSE, flag, lut_num);
			break;
...
void tune_cmc623_pwm_brightness(int value)
{
	...
	if (brightness_calls == 0) {
		...
		if (static_tune_loaded) {
			mDNIe_data_type *static_tune_mode;
...
static void init_mdnie_class(void)
{
	...
	if (device_create_file(mdnie_dev, &dev_attr_mdnieset_region_file_cmd) < 0)
