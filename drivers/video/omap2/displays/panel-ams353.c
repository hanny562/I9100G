...
#define LCD_XRES		480
#define LCD_YRES		800
#define LCD_HBP	        8 //16 //8   // Archer_LSJ DB07 : 16 => 8 
#define LCD_HFP         8 //16 //8   // Archer_LSJ DB07 : 16 => 8 
#define LCD_HSW		2   // 64  //3   // 2	
#define LCD_VBP		3   // 1   // 3 recomanded value in smd
#define LCD_VFP		28    //28
#define LCD_VSW		3    //3   // 2	

#define LCD_PIXCLOCK_MAX	      25690  //24380       
...
#ifdef  CONFIG_ARCHER_TARGET_SK  // KEY LED  // Archer_LSJ DA19
...
{
	printk(KERN_INFO " **** ams353_panel_probe.\n");
...
{
	printk(KERN_INFO " **** ams353_panel_suspend\n");
	spi_setup(ams353lcd_spi);

	ams353_panel_disable(dssdev);
...
{
	printk(KERN_INFO " **** ams353_panel_resume\n");
...
static void  ams353_panel_get_resolution(struct omap_dss_device *dssdev,
                u16 *xres, u16 *yres)
{
...
static void  ams353_panel_set_timings(struct omap_dss_device *dssdev,
                struct omap_video_timings *timings)
{
        dpi_set_timings(dssdev, timings);
}

static void  ams353_panel_get_timings(struct omap_dss_device *dssdev,
                struct omap_video_timings *timings)
{
        *timings = dssdev->panel.timings;
}

static int  ams353_panel_check_timings(struct omap_dss_device *dssdev,
                struct omap_video_timings *timings)
{
...
static struct omap_dss_driver ams353_driver = {
	...
	.get_resolution = ams353_panel_get_resolution,
	.suspend        = ams353_panel_suspend,
	.resume         = ams353_panel_resume,

        .set_timings    = ams353_panel_set_timings,
        .get_timings    = ams353_panel_get_timings,
        .check_timings  = ams353_panel_check_timings,
...
{
	printk(KERN_INFO " *** ams353_lcd_brightness_setting : %d, %d\n", idx, mode);
#ifndef LCD_ARIAN_GAMMA
	if ((idx > 15) || (idx < 0)) 
#else
	if ((idx > 27) || (idx < 0)) 
...
static void spi1writeindex(u8 index)
{
	...
	ret = spi_write(ams353lcd_spi,(unsigned char*)&cmd,2);
	
	if(ret < 0)
	{
		printk("[SHANKAR] SPI write error ret:%d \n", ret);
		return;
	}
	udelay(100);
	udelay(100);
...
static void spi1writedata(u8 data)
{
	...
	int ret =0;
	datas= 0x0100|data;
	ret = spi_write(ams353lcd_spi,(unsigned char*)&datas,2);
	if(ret < 0)
	{
		printk("[SHANKAR] SPI write error ret:%d \n", ret);
		return;
...
{
	printk(KERN_INFO" *** %s. brightness : %d, state : %d\n", __FUNCTION__, lcdlevel, on);
...
void ams353_ldi_poweron(void)
{
	printk(KERN_DEBUG " **** ams353_ldi_poweron. IN \n");
    ...
    #if 0  // Archer_LSJ DB07 : 16 => 8 
    spi1writedata(0x08);	// 4th HBP : 8 Dotclk
    spi1writedata(0x08);	// 5th HFP : 8 Dotclk
    #else 
	...
	printk(KERN_DEBUG " **** ams353_ldi_poweron. OUT \n");
}

void ams353_ldi_poweroff(void)
{
	printk(KERN_DEBUG " **** ams353_ldi_poweroff.\n");
...
static void ams353_set_brightness(struct led_classdev *dev, enum led_brightness value)
{
	...
	printk(KERN_DEBUG" *** ams353_set_brightness : %d\n", value);
...
{
	printk(KERN_INFO" *** ams353_set_lcd_gamma : %d\n", value);
...
{
	printk(KERN_INFO " **** ams353_spi_probe.\n");
	ams353lcd_spi = spi;
...
static int ams353_spi_suspend(struct spi_device *spi, pm_message_t mesg)
{
	...
	ams353lcd_spi = spi;
	ams353lcd_spi->mode = SPI_MODE_0;
	ams353lcd_spi->bits_per_word = 16 ;
	spi_setup(ams353lcd_spi);
