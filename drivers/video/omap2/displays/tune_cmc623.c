...
int omap_lcd_set_power(struct lcd_device *ld, int power)
{
	...
	if(power)
	{
			if(!lms700_state.powered_up)
			{
				...
				gpio_set_value(OMAP_GPIO_MLCD_ON, GPIO_LEVEL_HIGH);
	...
	else
	{
			...
			if(lms700_state.powered_up)
			{
				...
				mdelay(20);
				gpio_set_value(OMAP_GPIO_MLCD_ON, GPIO_LEVEL_LOW);
...
int tune_cmc623_suspend()
{
	...
	// VDD3 1.8V OFF
	gpio_direction_output(OMAP_GPIO_MLCD_ON1, GPIO_LEVEL_LOW);	
...
int tune_cmc623_pre_resume()
{
	...
	// VDD3 1.8V ON
	gpio_direction_output(OMAP_GPIO_MLCD_ON1, GPIO_LEVEL_HIGH);	
...
int tune_cmc623_resume()
{
	...
	// BYPASSB(A2) <= HIGH
	gpio_set_value(OMAP_GPIO_CMC_BYPASS, GPIO_LEVEL_LOW); // gpio_set_value(OMAP_GPIO_CMC_BYPASS, GPIO_LEVEL_HIGH);
	mdelay(1);
...
static int __devinit cmc623_probe(struct omap_dss_device *dssdev)
{
	...
	if (gpio_request(OMAP_GPIO_MLCD_ON1, "cmc_mlcd_on1") < 0) {
		printk(KERN_ERR "\n FAILED TO REQUEST GPIO %d for CMC623 \n", OMAP_GPIO_MLCD_ON1);
		return;
	}
	gpio_direction_output(OMAP_GPIO_MLCD_ON1, 1);	


	if (gpio_request(OMAP_GPIO_MLCD_ON, "cmc_enable") < 0) {
		printk(KERN_ERR "\n FAILED TO REQUEST GPIO %d for CMC623 \n", OMAP_GPIO_MLCD_ON);
		return;
	}
	gpio_direction_output(OMAP_GPIO_MLCD_ON, 1);
	...
	gpio_direction_output(OMAP_GPIO_CMC_BYPASS, 0); // gpio_direction_output(OMAP_GPIO_CMC_BYPASS, 1);

	/* LVDS SHDN ON */
	if (gpio_request(OMAP_GPIO_LVDS_SHDN, "lvds_shdn") < 0) {
		printk(KERN_ERR "\n FAILED TO REQUEST GPIO %d for CMC623 \n", OMAP_GPIO_LVDS_SHDN);
		return;
	}
	gpio_direction_output(OMAP_GPIO_LVDS_SHDN, 1); 
