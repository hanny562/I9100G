...
static int sharedregion_ioctl_setup(struct sharedregion_cmd_args *cargs)
{
	...
	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_ioctl_setup calling sharedregion_setup pid [%d] tgid [%d] comm [%s]\n", current->pid, current->tgid, current->comm);
	...
	for (i = 0; i < config.num_entries; i++) {
		...
		if (region.entry.is_valid == true) {
			...
			/*region.entry.base = MemoryOS_translate(
                ...
                //printk(KERN_ERR "SHARED_REG_INFO: Sharedregion_ioctl.c::sharedregion_ioctl_setup region.entry.base[0x%x]\n",region.entry.base);
                //printk(KERN_ERR "SHARED_REG_INFO: Sharedregion_ioctl.c::sharedregion_ioctl_setup region.entry.len[%d]\n",region.entry.len);
                //printk(KERN_ERR "SHARED_REG_INFO: Sharedregion_ioctl.c::sharedregion_ioctl_setup region.entry.owner_proc_id[0x%x]\n",region.entry.owner_proc_id);
                //printk(KERN_ERR "SHARED_REG_INFO: Sharedregion_ioctl.c::sharedregion_ioctl_setup region.entry.is_valid[0x%x]\n",region.entry.is_valid);
                //printk(KERN_ERR "SHARED_REG_INFO: Sharedregion_ioctl.c::sharedregion_ioctl_setup region.entry.cache_line_size[%d]\n",region.entry.cache_line_size);
                //printk(KERN_ERR "SHARED_REG_INFO: Sharedregion_ioctl.c::sharedregion_ioctl_setup region.entry.create_heap[0x%x]\n",region.entry.create_heap);
				//printk(KERN_ERR "SHARED_REG_INFO: sharedregion_ioctl_setup platform_mem_translate\n");
...
static int sharedregion_ioctl_destroy(
				struct sharedregion_cmd_args *cargs)
{
	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_ioctl_destroy calling sharedregion_destroy\n");
...
static int sharedregion_ioctl_set_entry(struct sharedregion_cmd_args *cargs)
{
	...
	//printk(KERN_ERR "SHARED_REG_INFO: sharedregion_ioctl_set_entry platform_mem_translate\n");
		...
		if (region.entry.is_valid == true) {
			...
			* addresses */
			/*region.entry.base = MemoryOS_translate (
					(Ptr)region.entry.base,
					Memory_XltFlags_Virt2Phys);*/
			//printk(KERN_ERR "SHARED_REG_INFO: sharedregion_ioctl_get_region_info platform_mem_translate region.entry.base [0x%x]\n", region.entry.base);
			region.entry.base = platform_mem_translate(
					region.entry.base,
					PLATFORM_MEM_XLT_FLAGS_VIRT2PHYS);
			if (region.entry.base == NULL) {
