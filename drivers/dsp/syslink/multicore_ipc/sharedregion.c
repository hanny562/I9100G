...
int sharedregion_setup(const struct sharedregion_config *config)
{
	...
	u32 i;
	s32 retval = 0;
	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_setup pid [%d] tgid [%d] comm [%s]\n", current->pid, current->tgid, current->comm);
	...
	*/
	...
	sharedregion_module->regions = kmalloc(
					...
					GFP_KERNEL);
	//printk(KERN_ERR "SHARED_REG_INFO: sharedregion_setup num_entries = [%d] regions = [0x%x]\n", sharedregion_module->cfg.num_entries, sharedregion_module->regions);
...
//	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_setup num_offset_bits = [%d] offset_mask = [ox%x]\n", sharedregion_module->num_offset_bits, sharedregion_module->offset_mask);
	...
	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_setup\n");
...
error:
	...
	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_setup calling sharedregion_destroy\n");
	sharedregion_destroy();
	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_setup retval = %d\n", retval);
	return retval;
}
...
int sharedregion_destroy(void)
{
	...
	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_destroy pid [%d] tgid [%d] comm [%s]\n", current->pid, current->tgid, current->comm);
	...
	if (retval)
		goto error;
	//printk(KERN_ERR "SHARED_REG_INFO: sharedregion_destroy freeing regions\n");
	kfree(sharedregion_module->regions);
	...
	kfree(local_lock);
	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_destroy\n");
	return 0;
...
error:
	...
	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_destroy retval = %d\n", retval);
	return retval;
}
...
int sharedregion_start(void)
{
	...
	int i;
	//printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_start\n");
		...
		if ((region->entry.is_valid)
			...
			params.shared_buf_size -=
				((heapmemmp_shared_mem_req(&params) - \
					params.shared_buf_size));
//			printk(KERN_ERR "SHARED_REG_INFO: sharedregion_start creating HeapMemMP for region->entry.base [0x%x] region->reserved_size [%d]\n", region->entry.base, region->reserved_size);
			heap_handle = heapmemmp_create(&params);
			if (heap_handle == NULL) {
//				printk(KERN_ERR "SHARED_REG_INFO: sharedregion_start creating HeapMemMP failed\n");
				retval = -1;
				break;
			} else {
...
error:
	...
	//printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_start retval = %d\n", retval);
	return retval;
}
...
int sharedregion_stop(void)
{
	...
	struct sharedregion_region *region = NULL;
	int i;
	//printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_stop\n");
	...
	for (i = 0; i < sharedregion_module->cfg.num_entries; i++) {
		...
		if ((region->entry.is_valid)
			...
			&& (region->heap != NULL)) {
			/* Delete heap */
//			printk(KERN_ERR "SHARED_REG_INFO: sharedregion_stop deleting heap for region->entry.base [0x%x]\n", region->entry.base);
			tmp_status = heapmemmp_delete((void **)&(region->heap));
			if ((tmp_status < 0) && (retval >= 0)) {
				retval = -1;
//				printk(KERN_ERR "SHARED_REG_INFO: sharedregion_stop deleting heap falied\n"); 
			}
		}
...
error:
	...
	//printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_stop retval = %d\n", retval);
	return retval;
}
...
int sharedregion_attach(u16 remote_proc_id)
{
	...
	//printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_attach\n");
	if (WARN_ON(atomic_cmpmask_and_lt(&(sharedregion_module->ref_count),
				SHAREDREGION_MAKE_MAGICSTAMP(0),
				SHAREDREGION_MAKE_MAGICSTAMP(1)) == true)) {
		retval = -ENODEV;
...
//			printk(KERN_ERR "SHARED_REG_INFO: sharedregion_attach region->entry.base [0x%x] region->reserved_size [%d] shared_addr [0x%x]\n", region->entry.base, region->reserved_size, shared_addr);
			/* Heap should already be created so open by address */
			retval = heapmemmp_open_by_addr(shared_addr,
						(void **) &(region->heap));
			if (retval < 0) {
//				printk(KERN_ERR "SHARED_REG_INFO: sharedregion_attach failed\n"); 
				retval = -1;
				break;
			}
		}
...
error:
	...
	//printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_attach retval = %d\n", retval);
	return retval;
}
...
int sharedregion_detach(u16 remote_proc_id)
{
	...
	struct sharedregion_region *region = NULL;
	u16 i;
	//printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_deattach\n");
...
//			printk(KERN_ERR "SHARED_REG_INFO: sharedregion_deattach region->entry.base [0x%x] region->reserved_size [%d]\n", region->entry.base, region->reserved_size);
			if ((tmp_status < 0) && (retval >= 0)) {
//				printk(KERN_ERR "SHARED_REG_INFO: sharedregion_deattach failed\n");
				retval = -1;
...
error:
	...
	//printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_deattach retval = %d\n", retval);
	return retval;
}
...
void *sharedregion_get_ptr(u32 *srptr)
{
	...
	u16 region_id;
	s32 retval = 0;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_get_ptr\n");
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_get_ptr retptr [0x%x]\n", return_ptr);
	return return_ptr;

error:
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_get_ptr failed retval = %d\n", retval);
...
u32 *sharedregion_get_srptr(void *addr, u16 id)
{
	...
	s32 retval = 0;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_get_srptr\n");
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_get_srptr ret_ptr [0x%x]\n", ret_ptr);
	return ret_ptr;

error:
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_get_srptr retval = %d\n", retval);
...
void sharedregion_get_region_info(u16 id, struct sharedregion_region *region)
{
	...
	s32 retval = 0;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_get_region_info\n");
	...
	memcpy((void *) region, (void *) regions,
		sizeof(struct sharedregion_region));
//n	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_get_region_info region->entry.base [0x%x]\n", region->entry.base);
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_get_region_info retval = %d\n", retval);
	return;
}
...
int sharedregion_set_entry(u16 id, struct sharedregion_entry *entry)
{
	...
	struct heapmemmp_params params;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_set_entry\n");
	...
	region = &(sharedregion_module->regions[id]);
//	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_set_entry region->entry.base [0x%x]\n", region->entry.base);

	/* Make sure region does not overlap existing ones */
	retval = _sharedregion_check_overlap(region->entry.base,
						region->entry.len);
	if (retval < 0) {
//		printk(KERN_ERR "SHARED_REG_INFO: sharedregion_set_entry region->entry.base [0x%x] overlaps with existing entry\n", region->entry.base);
...
//		printk(KERN_ERR "SHARED_REG_INFO: sharedregion_set_entry region->entry.base [0x%x] already exists\n", region->entry.base);
		goto error;
	}
...
//		printk(KERN_ERR "SHARED_REG_INFO: sharedregion_set_entry region->entry.base [0x%x] cache enabled but linesize not zero\n", region->entry.base);
		goto error;
	}
...
//			printk(KERN_ERR "SHARED_REG_INFO: sharedregion_set_entry region->entry.base [0x%x] calling heapmemmp_create with shared_addr [0x%x]\n", region->entry.base, params.shared_addr);
			heap_handle = heapmemmp_create(&params);
			if (heap_handle == NULL) {
//				printk(KERN_ERR "SHARED_REG_INFO: sharedregion_set_entry region->entry.base [0x%x] heapmemmp_create failed\n", region->entry.base);
...
//			printk(KERN_ERR "SHARED_REG_INFO: sharedregion_set_entry region->entry.base [0x%x] calling heapmemmp_open_by_addr with shared_addr [0x%x]\n", region->entry.base, params.shared_addr);
			/* open the heap by address */
			retval = heapmemmp_open_by_addr(shared_addr, (void **)
							heap_handle_ptr);
			if (retval < 0) {
//				printk(KERN_ERR "SHARED_REG_INFO: sharedregion_set_entry region->entry.base [0x%x] heapmemmp_open_by_addr failed\n", region->entry.base);
				region->entry.is_valid = false;
				retval = -1;
				goto error;
			}
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_set_entry\n");
	return 0;

error:
	pr_err("sharedregion_set_entry failed! status = 0x%x", retval);
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_set_entry retval = %d\n", retval);
	return retval;
}
...
int sharedregion_clear_entry(u16 id)
{
	...
	u16 my_id;
	u16 owner_proc_id;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_clear_entry\n");
	...
	if (retval)
		goto error;
	region = &(sharedregion_module->regions[id]);
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_clear_entry region->entry.base [0x%x]\n", region->entry.base);
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_clear_entry retval = %d\n", retval);
	return retval;
}
...
void sharedregion_clear_reserved_memory(void)
{
	struct sharedregion_region *region = NULL;
	int i;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_clear_reserved_memory\n");
	...
	for (i = 0; i < sharedregion_module->cfg.num_entries; i++) {
		...
		if ((region->entry.is_valid) && \
			(region->entry.owner_proc_id == multiproc_self())) {
//			printk(KERN_ERR "SHARED_REG_INFO: sharedregion_clear_reserved_memory region->entry.base [0x%x]\n", region->entry.base);
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_clear_reserved_memory\n");
}
...
{
	s32 retval = 0;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_entry_init\n");
...
//	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_entry_init region->entry.base [0x%x]\n", entry->base);
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_entry_init retval = %d\n", retval);
	return;
}
...
int sharedregion_get_entry(u16 id, struct sharedregion_entry *entry)
{
	int retval = 0;
	struct sharedregion_region *region = NULL;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_get_entry\n");
	...
	region = &(sharedregion_module->regions[id]);
//	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_get_entry region->entry.base [0x%x]\n", region->entry.base);
	memcpy((void *) entry, (void *) &(region->entry),
			sizeof(struct sharedregion_entry));
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_get_entry\n");
	return 0;

error:
	pr_err("sharedregion_get_entry failed: status = 0x%x", retval);
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_get_entry retval = %d\n", retval);
	return retval;
}
...
void *sharedregion_reserve_memory(u16 id, uint size)
{
	...
	s32 retval = 0;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_reserve_memory\n");
	...
	region = &(sharedregion_module->regions[id]);
//	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_reserve_memory region->entry.base [0x%x]\n", region->entry.base);
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_reserve_memory region->entry.base [0x%x] ret_ptr [0x%x]\n", region->entry.base, ret_ptr);
	return ret_ptr;

error:
	pr_err("sharedregion_reserve_memory failed: status = 0x%x", retval);
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_reserve_memory retval = %d\n", retval);
	return (void *)NULL;
}
...
void sharedregion_unreserve_memory(u16 id, uint size)
{
	...
	uint cache_line_size = 0;
	s32 retval = 0;
//	printk(KERN_ERR "SHARED_REG_ENTRY: sharedregion_unreserve_memory\n");
	...
	region = &(sharedregion_module->regions[id]);
//	printk(KERN_ERR "SHARED_REG_INFO: sharedregion_unreserve_memory region->entry.base [0x%x]\n", region->entry.base);
...
//	printk(KERN_ERR "SHARED_REG_EXIT: sharedregion_unreserve_memory retval = %d\n", retval);
	return;
}
...
int _sharedregion_set_entry(u16 id, struct sharedregion_entry *entry)
{
	int retval = 0;
	struct sharedregion_region *region = NULL;
//	printk(KERN_ERR "SHARED_REG_ENTRY: _sharedregion_set_entry doesnt create heap\n");
	...
	region = &(sharedregion_module->regions[id]);
//	printk(KERN_ERR "SHARED_REG_INFO: _sharedregion_set_entry region->entry.base [0x%x]\n", region->entry.base);
...
//	printk(KERN_ERR "SHARED_REG_EXIT: _sharedregion_set_entry retval = %d\n", retval);
	return retval;
}
