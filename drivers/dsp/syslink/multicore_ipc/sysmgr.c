...
s32 sysmgr_setup(const struct sysmgr_config *cfg)
{
	...
	printk(KERN_ERR "SHARED_REG_INFO: sysmgr_setup platform_mem_setup\n");
	...
	if (status >= 0) {
		...
		printk(KERN_ERR "SHARED_REG_INFO: sysmgr_setup platform_setup\n");
...
s32 sysmgr_destroy(void)
{
	...
	if (sysmgr_state.platform_init_flag == true) {
		printk(KERN_ERR "SHARED_REG_INFO: sysmgr_destroy platform_destroy\n");
	...
	if (sysmgr_state.sharedregion_init_flag == true) {
		printk (KERN_ERR "SHARED_REG_INFO: sysmgr_destroy calling sharedregion_destroy\n");
	...
	if (sysmgr_state.platform_mem_init_flag == true) {
		...
		printk(KERN_ERR "SHARED_REG_INFO: sysmgr_destroy platform_mem_destroy\n");
