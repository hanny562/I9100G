...
platform_setup(void)
{
	...
	printk(KERN_ERR "SHARED_REG_ENTRY: platform_setup pid [%d] tgid [%d] comm [%s]\n", current->pid, current->tgid, current->comm);
	platform_get_config(&_config);
	config = &_config;

	/* Initialize PlatformMem */
	printk(KERN_ERR "SHARED_REG_INFO: platform_setup platform_mem_setup\n");
	...
	if (status >= 0) {
		...
		//printk(KERN_ERR "SHARED_REG_INFO: platform_setup platform_mem_map initializing GateHWSpinlock src 0x4A0F6000 size 0x1000\n");
	...
	printk(KERN_ERR "SHARED_REG_EXIT: platform_setup\n");
	return status;
}
...
platform_destroy(void)
{
	...
	printk(KERN_ERR "SHARED_REG_ENTRY: platform_destroy pid [%d] tgid [%d] comm [%s]\n", current->pid, current->tgid, current->comm);
	...
	if (platform_module->gatehwspinlock_init_flag == true) {
		...
		//printk(KERN_ERR "SHARED_REG_INFO: platform_destroy platform_mem_unmap finalizing GateHWSpinlock src 0x4A0F6000 size 0x1000\n");
	...
	if (platform_module->sharedregion_init_flag == true) {
		printk(KERN_ERR "SHARED_REG_INFO: platform_destroy calling sharedregion_destroy\n");
	...
	if (platform_module->platform_mem_init_flag == true) {
		printk(KERN_ERR "SHARED_REG_INFO: platform_destroy platform_mem_init_falg is true calling platform_mem_destroy\n");
	...
	printk(KERN_ERR "SHARED_REG_EXIT: platform_destroy status = %d\n", status);
	return status;
}
...
{
	int status = PLATFORM_S_SUCCESS;
	printk(KERN_ERR "WA_INFO: platform_start_callback pid [%d] tgid [%d] comm [%s]\n", current->pid, current->tgid, current->comm);
	...
	printk(KERN_ERR "WA_INFO: platform_start_callback status = %d\n", status);
	if (status < 0)
		pr_err("platform_load_callback failed, status [0x%x]\n",
			status);
	printk(KERN_ERR "WA_INFO: platform_start_callback calling IPC_START\n");
	pr_err("Calling ipc_notify_event with IPC_START in platform_start_callback\n");
