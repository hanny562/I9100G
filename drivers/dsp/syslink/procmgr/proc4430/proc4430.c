...
int proc4430_attach(void *handle, struct processor_attach_params *params)
{
	...
	printk(KERN_ERR "SHARED_REG_ENTRY: proc4430_attach pid [%d] tgid [%d] comm [%s]\n", current->pid, current->tgid, current->comm);
    ...
    printk(KERN_ERR "SHARED_REG_ENTRY:[%s]Proc4430.c::proc4430_attach before incr attach_count[0x%x]\n", current->comm, atomic_read(&object->attach_count));
	atomic_cmpmask_and_set(&object->attach_count,
				OMAP4430PROC_MAKE_MAGICSTAMP(0),
				OMAP4430PROC_MAKE_MAGICSTAMP(0));
	atomic_inc_return(&object->attach_count);
    printk(KERN_ERR "SHARED_REG_ENTRY:[%s]Proc4430.c::proc4430_attach after incr attach_count[0x%x]\n", current->comm, atomic_read(&object->attach_count));
	...
	for (i = 0; (i < object->params.num_mem_entries); i++) {
		...
		*/
		...
		&& (object->params.mem_entries[i].shared == true)) {
			...
			//printk(KERN_ERR "SHARED_REG_INFO: proc4430_attach map_info.src [0x%x] map_info.size [0x%x]\n", map_info.src, map_info.size);
			retval = platform_mem_map(&map_info);
			if (retval != 0) {
				//printk(KERN_ERR "SHARED_REG_INFO: proc4430_attach platform_mem_map failed\n");
				pr_err("proc4430_attach failed\n");
				return -EFAULT;
			}
			map_count++;
	...
	printk(KERN_ERR "SHARED_REG_EXIT: proc4430_attach\n");
	return retval;
}
...
int proc4430_detach(void *handle)
{
	...
	printk(KERN_ERR "SHARED_REG_ENTRY: proc4430_detach pid [%d] tgid [%d] comm [%s]\n", current->pid, current->tgid, current->comm);
	...
	proc_handle = (struct processor_object *)handle;
	object = (struct proc4430_object *)proc_handle->object;
    printk(KERN_ERR "SHARED_REG_ENTRY:[%s]Proc4430.c::proc4430_detach before decr attach_count[0x%x]\n", current->comm, atomic_read(&object->attach_count));
	if (!(atomic_dec_return(&object->attach_count) == \
		OMAP4430PROC_MAKE_MAGICSTAMP(0))) {
        printk(KERN_ERR "SHARED_REG_ENTRY:[%s]Proc4430.c::proc4430_detach after decr attach_count not 0[0x%x]\n", current->comm, atomic_read(&object->attach_count));
		return 1;
	}
    printk(KERN_ERR "SHARED_REG_ENTRY:[%s]Proc4430.c::proc4430_detach after decr attach_count[0x%x]\n", current->comm, atomic_read(&object->attach_count));
	for (i = 0; (i < object->params.num_mem_entries); i++) {
	...
	printk(KERN_ERR "SHARED_REG_EXIT: proc4430_detach\n");
	return 0;
}
