...
#define TWL6030_REG_SYSTEN_CFG_STATE	0xB5
...
struct omap_hsmmc_host *omap_hsmmc_host_pub = NULL;
...
u8 omap_mmc2_ldo_status(void)
{
	u8 value;
	twl_i2c_read_u8(TWL6030_MODULE_ID0, &value, TWL6030_REG_SYSEN_CFG_STATE);
	printk(" \nPower status of MMC2 LDO in SYSEN = 0x%02x \n",value);
	return value;
}

EXPORT_SYMBOL(omap_mmc2_ldo_status);

static int omap_hsmmc_Movinand_set_power(struct device *dev, int slot, int power_on, int vdd)
{
	int ret = 0;
	...
	if(power_on)
	{
		/* Turn on the external LDO to eMMC */
		twl_i2c_write_u8(TWL6030_MODULE_ID0,
			STATE_MOD_APP|STATE_MOD_CON|STATE_MOD_MOD|STATE_MOD_ON,
			TWL6030_REG_SYSEN_CFG_STATE);
		printk(KERN_ERR "Turn ON External LDO \n");
	}
	else
	{
		...
		twl_i2c_write_u8(TWL6030_MODULE_ID0,
			...
			TWL6030_REG_SYSEN_CFG_STATE);
		printk(KERN_ERR "Turn OFF External LDO \n");
...
static int omap_hsmmc_reg_get(struct omap_hsmmc_host *host)
{
	...
	case OMAP_MMC2_DEVID:
		printk("%s with id%d is registered with Movinand_set_power \n",
			mmc_hostname(host->mmc), host->id);
#if 0
		twl_i2c_write_u8(TWL6030_MODULE_ID0, GROUP_APP|GROUP_CON|GROUP_MOD,
		...
		mmc_slot(host).set_power = omap_hsmmc_Movinand_set_power;
		break;
			     ...
			     unsigned int timeout_clks)
{
	...
	if(host->mmc->index==1 || host->mmc->index== 0)
		clk_rate=96000000;
	if(host->mmc->index==2)
		clk_rate= 48000000;

	//omap_writel(0x1040002,0x4A009330);
	//omap_writel(0x1040002,0x4A009328);
			
	reg = OMAP_HSMMC_READ(host, SYSCTL);
	clkd = (reg & CLKD_MASK) >> CLKD_SHIFT;
	if (clkd == 0)
		clkd = 1;
	cycle_ns = 1000000000 / (clk_rate / clkd);
...
#define MMC_CLK_RECOVERY_LOGIC

static void omap_hsmmc_request(struct mmc_host *mmc, struct mmc_request *req)
{
	struct omap_hsmmc_host *host = mmc_priv(mmc);
	int err;
	unsigned int reg_val,tmp_val;
...
/**************************************************************************
* 
* This Below code is added while debugging the Kernel Panics due to 
* MMC clock disable during T1 product.
* 
* Change Begin Here (2011/06/02)
**************************************************************************/
#ifdef MMC_CLK_RECOVERY_LOGIC
		...
		case OMAP_MMC1_DEVID:
			 ...
			 reg_val = omap_readl(0x4A009328);
		        ...
		        {
		          printk("HSMMC1_CLK Not enabled=0x%X\n",reg_val);
		          do {
		            omap_writel((reg_val | 0x02),0x4A009328);
		            udelay(10);
			        tmp_val = omap_readl(0x4A009328);
			        tmp_val=(tmp_val&0x30000)>>16;
					if((tmp_val == 0x0) || (tmp_val == 0x2))
					   break;
			      } while(1);
			      printk("HSMMC1_CLK re-enabled=0x%X\n",reg_val);
		        }
			break;
		...
		case OMAP_MMC2_DEVID:
		        ...
		        if((tmp_val == 0x03)||(tmp_val ==0x01))
		        {
		          printk("HSMMC2_CLK Not enabled=0x%X\n",reg_val);
		          do {
		            omap_writel((reg_val | 0x02),0x4A009330);
		            udelay(10);
			        tmp_val = omap_readl(0x4A009330);
			        tmp_val=(tmp_val&0x30000)>>16;
					if((tmp_val == 0x0) || (tmp_val == 0x2))
					   break;
			      } while(1);
			      printk("HSMMC2_CLK re-enabled=0x%X\n",reg_val);
		        }
				break;
		case OMAP_MMC5_DEVID:
			...
			reg_val = omap_readl(0x4a009560);
		        ...
		        if((tmp_val == 0x03)||(tmp_val ==0x01))
		        {
		          printk("HSMMC5_CLK Not enabled for WIFI=0x%X\n",reg_val);
		          do {
		               omap_writel((reg_val | 0x02),0x4a009560);
		               udelay(10);
			        tmp_val = omap_readl(0x4a009560);
			        tmp_val=(tmp_val&0x30000)>>16;
					if((tmp_val == 0x0) || (tmp_val == 0x2))
					   break;
			      } while(1);
			      printk("HSMMC5_CLK re-enabled for WIFI =0x%X\n",reg_val);
		        }
				break;
		...
		case OMAP_MMC4_DEVID:
		default:
			printk("**** ERROR: Unknown MMC Host ID=%d ****\n",host->id);
			break;
		
		}

#endif //MMC_CLK_RECOVERY_LOGIC	      
/**************************************************************************
* 
* This Below code is added while debugging the Kernel Panics due to 
* MMC clock disable during T1 product.
...
void omap_hsmmc_force_presence_change_for_wlan(int on)
{
	printk(KERN_WARNING"%s: enter\n", __func__);
	
	if (omap_hsmmc_host_pub) {
		omap_hsmmc_host_pub->mmc->pm_flags |= MMC_PM_KEEP_POWER;
	
		if(on)
			mmc_detect_change(omap_hsmmc_host_pub->mmc, msecs_to_jiffies(500));
		else
			mmc_detect_change(omap_hsmmc_host_pub->mmc, msecs_to_jiffies(100));

		printk(KERN_DEBUG"%s: mmc detect change called, "
			"omap_hsmmc_host_pub=%p\n", __func__, omap_hsmmc_host_pub);
		
	} else {
		printk(KERN_ERR"%s: null pointer, omap_hsmmc_host_pub=%p\n", 
			__func__, omap_hsmmc_host_pub);
	}
}
EXPORT_SYMBOL(omap_hsmmc_force_presence_change_for_wlan);


void omap_hsmmc_force_presence_change(void)
{
	printk(KERN_WARNING"%s: enter\n", __func__);
	...
	if (omap_hsmmc_host_pub) {
		...
		mmc_detect_change(omap_hsmmc_host_pub->mmc, msecs_to_jiffies(200));
...
static int __init omap_hsmmc_probe(struct platform_device *pdev)
{
	...
	if (OMAP_MMC5_DEVID == host->id) {
		omap_hsmmc_host_pub = host;
		printk(KERN_INFO"%s: mmc host global pointer stored, " 
			"omap_hsmmc_host_pub=%p\n", __func__, omap_hsmmc_host_pub); 
	}
