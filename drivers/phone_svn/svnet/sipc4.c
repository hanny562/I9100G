...
{
	printk("cp state change. state : %d\n", state);
...
static int _write_raw_skb(struct ringbuf *rb, int res, struct sk_buff *skb)
{
	...
	if (!cp_state) {
		printk("_write_raw_skb : cp_state : %d\n", cp_state);
		return -EPERM;
	}
...
static int _write_rfs_skb(struct ringbuf *rb, struct sk_buff *skb)
{
	...
	if (!cp_state) {
		printk("_write_rfs_skb : cp_state : %d\n", cp_state);
		return -EPERM;
	}
		...
		u8 control)
{
	...
	if (!cp_state) {
		printk("_write_fmt_buf : cp_state : %d\n", cp_state);
		return -EPERM;
	}
		...
		struct rfs_hdr *h)
{
	...
	if (!cp_state) {
		printk("_read_rfs_data : cp_state : %d\n", cp_state);
		return -EPERM;
	}
...
static void test_copy_buf(struct sipc *si, int idx)
{
	...
	if (!cp_state) {
		printk("test_copy_buf : cp_state : %d\n", cp_state);
		return;
	}
