			   ...
			   const struct i2c_device_id *id)
{
	...
	if (client->irq) {
		...
		ret =
			request_irq(client->irq, low_battery_isr, IRQF_DISABLED,
				    client->name, NULL);
		if (ret) {
...
static int fuelgauge_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (!sec_bci->charger.is_charging) {
		...
		if (batt_ptg > BATTERY_LOW_PERCENTAGE)
			...
			alert_th = 0x1C;	// 4%
		else if (batt_ptg > BATTERY_POWEROFF_PERCENTAGE)
			alert_th = 0x1E;	// 2%
		else
			alert_th = 0x1F;	// 1%
...
int fuelgauge_init(void)
{
	...
	fuelgauge_i2c_client =
		omap_gpio_i2c_init(OMAP_GPIO_FUEL_SDA, OMAP_GPIO_FUEL_SCL, 0x36,
				   100);
